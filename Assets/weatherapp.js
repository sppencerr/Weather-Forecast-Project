$(document).ready(function () {
 ...
 // Functions to build the URL for the OpenWeather API call
  ...
  function searchWeather(queryURL) {
      ...
      $.ajax({
          ...
          $.ajax({
              ...
              // display 5 day forecast in the elements from dom
              for (let i = 0; i <= 5; i++) {
  ...
  // on click button from handler
  $('#search-btn').on('click', function (event) {
      // prevent default (submitting)
      event.preventDefault();

      // retrieve city input and scrub
  ...
  // upload any cities in local storage into array
  loadCities();
  displayCities(previousCities);
